...
const GistDetails = ({ gist, className }) => <div className={className}>
  <div>
    ...
    <span
      className={styles.value}>{gist.get('description') && gist.get('description').length ? gist.get('description') : 'No description provided.'}</span>
  </div>
  <div>
    <span className={styles.label}>Created At :</span>
    <span className={styles.value}> {getDate(gist.get('created_at'))}</span>
  </div>
  <div>
